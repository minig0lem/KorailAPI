    ...
    public CrawlingResponse crawlNeilroTrain(NeilroRequest neilroRequest) {
        ...
        for(int i = 0; i < linkSize; i++) {
            ...
            for (int j = 1; j <= roomSize; j++) {
                ...
                //총 예매 가능 좌석 가져오기
                List<WebElement> availableSeats = driver.findElements(By.xpath("//div[contains(@class, 'seat_box_in')]//span[contains(@class, 'on')]"));
                sum += availableSeats.size();
            }
            log.info("열차번호 {}의 총 예매 가능 좌석 수 는 {} 입니다.", trainNum, sum);
            //열차 시간표 페이지로 프레임 전환
